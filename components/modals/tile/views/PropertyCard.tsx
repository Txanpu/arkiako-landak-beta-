        ...
        <div className="bg-slate-900 text-white w-full max-w-sm rounded-xl overflow-hidden shadow-2xl border border-slate-700 animate-in zoom-in-95" onClick={e => e.stopPropagation()}>
            ...
            <div className="p-6 space-y-4 bg-slate-900">
                    ...
                    <table className="w-full">
                        ...
                        <tbody className="divide-y divide-slate-700/50">
                            {getRentTable(t).map((row, idx) => {
                                // Determine if this row is the current level
                                let isActive = false;
                                if (!t.mortgaged) {
                ...
                <div className="grid grid-cols-2 gap-2 mt-2">
                    {canBuyDirect && <button onClick={() => {dispatch({type: 'BUY_PROP'}); dispatch({type: 'CLOSE_MODAL'})}} className="col-span-2 bg-green-600 hover:bg-green-500 py-3 rounded-lg font-black text-white shadow-lg border-b-4 border-green-800 active:scale-95 transition-all">COMPRAR</button>}
                    {canAuction && <button onClick={() => {dispatch({type: 'START_AUCTION', payload: t.id}); dispatch({type: 'CLOSE_MODAL'})}} className="col-span-2 bg-purple-600 hover:bg-purple-500 py-3 rounded-lg font-black text-white shadow-lg border-b-4 border-purple-800 active:scale-95 transition-all">SUBASTAR</button>}
